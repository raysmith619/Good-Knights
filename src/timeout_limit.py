...
def time_limit(time_len):
    def deco(func):
        ...
        def wrapper(*args, **kwargs):
            res = [Exception('function [%s] timeout [%s seconds] exceeded!' % (func.__name__, time_len))]
            def newFunc():
                try:
...
if __name__ == "__main__":
    ...
    def long_fun(timein=None):
        ...
        for _ in range(timein):
            print("timein sleep(1)")
            time.sleep(1)
        print("timein end")
        
    time_lim = 3    
    timed_long_fun = time_limit(time_lim)(long_fun)
